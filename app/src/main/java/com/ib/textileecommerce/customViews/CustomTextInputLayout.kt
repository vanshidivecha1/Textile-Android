...
class CustomTextInputLayout(context: Context, attrs: AttributeSet) : TextInputLayout(context, attrs) {
    ...
    private fun setTypeface(context: Context, attrs: AttributeSet) {
        val values = context.obtainStyledAttributes(attrs, R.styleable.CustomTextInputLayout)

        val typefaceValue = values.getInt(R.styleable.CustomTextInputLayout_typeface, 0)
        values.recycle()
